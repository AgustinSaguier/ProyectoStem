...
with detection_graph.as_default():
    with tf.Session(graph=detection_graph) as sess:
        while True:
            ...
            detected = ([category_index.get(value) for index,value in enumerate(classes[0]) if scores[0,index] > 0.5])

            if detected!=[]:
                detectedObj = detected[0]
                detectedName = detectedObj['name']
                print (detectedName)

                if detectedName == "Garbage bag":
                    arduino.write(b'1')        
